...
class XXX_Norm8(nn.BatchNorm1d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True):
        super(XXX_Norm8, self).__init__(num_features, eps, momentum, affine, track_running_stats)
        if self.affine:
        ...
        self.npower_weight = nn.Parameter(torch.zeros(num_features))
    ...
    def forward(self, graph, tensor):  
        ...
        weight_scales = graph.ndata['node_weight_g_normed']*torch.pow(graph.ndata['sg_nodes'].repeat(1,self.num_features), self.npower_weight)
        ...
        sacle_factor = torch.sigmoid(weight_scales)
