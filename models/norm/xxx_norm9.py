...
class XXX_Norm9(nn.BatchNorm1d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
                 track_running_stats=True):
        super(XXX_Norm9, self).__init__(num_features, eps, momentum, affine, track_running_stats)
        if self.affine:
    ...
    def forward(self, graph, tensor):  
        ...
        results = results + self.fea_scale_weight*repeat_tensor_interleave(graph_mean+batch_mean, graph.batch_num_nodes())
        # if self.affine:
