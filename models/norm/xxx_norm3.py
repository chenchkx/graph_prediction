...
class XXX_Norm3(nn.BatchNorm1d):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        self.var_bias_weight = nn.Parameter(torch.ones(num_features))
    ...
    def forward(self, graph, tensor):  
        ...
        var_scale = segment_repeat(torch.sqrt(batch_var/(segment_reduce(batch_num_nodes, torch.pow(results,2),reducer='mean')+self.eps)), batch_num_nodes)   
        var_scale = torch.sigmoid(self.var_scale_weight*var_scale+self.var_bias_weight)*graph.ndata['degrees_normed'].unsqueeze(1)
