...
class XXX_Norm7(nn.BatchNorm1d):
    ...
    def forward(self, graph, tensor):
        ...
        results = torch.sigmoid(self.latent_energy*self.running_var)*results

        graph_mean = segment.segment_reduce(graph.batch_num_nodes(), results, reducer='mean')
        results = results + repeat_tensor_interleave(torch.tanh(self.graph_project_weight)*graph_mean+self.graph_project_bias, graph.batch_num_nodes())
